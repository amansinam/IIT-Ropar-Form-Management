...
import { EyeOff, Loader2, GraduationCap, Shield, AlertCircle } from 'lucide-react';
import { toast } from 'sonner';
import { useSession, signOut } from 'next-auth/react';
import { AuthService, VERIFIER_ROLES, VerifierRole } from '@/lib/auth.service';
...
function LoginForm() {
  ...
  useEffect(() => {
    ...
    const role = session.user.role as VerifierRole | undefined;
    const portal = session.user.portal as string | undefined;

    const isValidRole = role && VERIFIER_ROLES.includes(role);
    const isValidPortal = portal === 'verifier';

    if (isValidRole && isValidPortal) {
      // ✅ Valid verifier — redirect to dashboard
      toast.success(`Welcome back, ${session.user.name?.split(' ')[0] ?? 'Verifier'}!`);
      router.replace('/dashboard');
    } else {
      // ❌ Wrong role or wrong portal — sign out silently and show banner
  ...
  useEffect(() => {
    ...
    if (role && VERIFIER_ROLES.includes(role)) {
      // ✅ Valid verifier role (HOD, Caretaker, Dean, Faculty)
      toast.success(`Welcome back, ${session.user.name?.split(' ')[0] ?? 'Verifier'}!`);
      router.replace('/dashboard');
    } else {
      // ❌ Signed in but role is not a valid verifier role — sign out silently
      setUnauthorized(true);
      signOut({ redirect: false });
    }
  }, [status, session, router]);

  // ── Spinner while loading or while valid user is being redirected ───
  if (
    status === 'loading' ||
    (
      status === 'authenticated' &&
      VERIFIER_ROLES.includes(session?.user?.role as VerifierRole) &&
      session?.user?.portal === 'verifier'   // ← add portal check here too
    )
  ) {
    return (
        ...
        <div className="relative z-10 space-y-8">
          <div>
            <h1 className="text-white text-4xl font-bold leading-tight mb-4" style={{ fontFamily: 'Poppins, sans-serif' }}>
              Form Verification<br />Portal
            </h1>
            <p className="text-blue-200 text-base leading-relaxed">
              Streamline document verification workflows across departments with a centralized platform built for IIT Ropar.
            </p>
          </div>
          ...
          <div className="space-y-4">
            {[
              ...
              { icon: <EyeOff size={20} />, title: 'Role-based Access', desc: 'Each verifier sees only their queue' },
        ...
        <div className="w-full max-w-md">
          ...
          <div className="lg:hidden flex items-center gap-3 mb-8">
            ...
            <div>
              ...
              <p className="text-gray-500 text-xs">Form Verification Portal</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-8 shadow-xl border border-gray-100">
            <div className="mb-8">
              ...
              <p className="text-gray-500 text-sm">Sign in to access the verification portal</p>
            </div>

            {/* ── Unauthorized banner — role not in VERIFIER_ROLES ── */}
            {unauthorized && (
              <div className="mb-5 p-4 bg-red-50 border border-red-200 rounded-xl flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-red-500 text-xs mt-0.5 leading-relaxed">
                    You are not authorized to access this portal. Only registered IIT Ropar verifier accounts (HOD, Dean, Caretaker, Faculty) are permitted. Please contact your system administrator.
                  </p>
                </div>
              </div>
            )}

            {/* ── URL error banner — from ?error= param ── */}
            {!unauthorized && searchParams.get('error') && (
              <div className="mb-5 p-4 bg-red-50 border border-red-200 rounded-xl flex items-start gap-3">
                <div className="w-8 h-8 bg-red-100 rounded-lg flex items-center justify-center flex-shrink-0">
                  <AlertCircle size={16} className="text-red-500" />
                </div>
                <div>
                  ...
                  <p className="text-red-500 text-xs mt-0.5 leading-relaxed">
                    Your Google account is not registered as a verifier. Contact your system administrator.
                  </p>
                </div>
              </div>
            )}
            ...
            <p className="text-center text-gray-400 text-xs mt-4 leading-relaxed">
              Only registered IIT Ropar verifier accounts can access this portal.
            </p>
          </div>
        </div>
      </div>
